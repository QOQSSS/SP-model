...
def set_objective(mdl, fuel, fuel_cost1, ship_count, fixed_cost1, total_fuel_cost, total_fixed_cost, cargo1, revenue1,
    ...
    fixed_cost_sum = fixed_cost1 * ship_count  
    ...
    for j in range(N):
        for i in range(len(segments1)):
            ...
            mdl.addGenConstrPWL(speed[i+j*n], v_coef1, v_points1, coefficients1,
                                name=f"pwl_coef1_{i}")  
            mdl.addConstr(fuel_consume[i+j*n] == distance1[i] * v_coef1, "circleConstraint")  # 分段函数描述的是燃油量与航速之间的关系
...
def main():
    ...
    #with open(args.params file,'r')as f:params = json.load(f)
